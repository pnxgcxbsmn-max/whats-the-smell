...
/* ========================= Wikipedia/Wikidata + images (legacy endpoints) ========================= */
app.get("/api/wiki-summary", async (req, res) => {
  try {
    const q = String(req.query?.q || "").trim();
    const L = (String(req.query?.lang || "en").trim() || "en").toLowerCase();

    if (!q) return res.status(400).json({ error: "Missing q" });

    const searchUrl = `https://${L}.wikipedia.org/w/rest.php/v1/search/title?q=${encodeURIComponent(q)}&limit=1`;
    const s = await fetch(searchUrl);
    const sData = await safeJson(s);
    const title = sData?.pages?.[0]?.title;

    if (!title) return res.json({ title: "", extract: "" });

    const sumUrl = `https://${L}.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(title)}`;
    const r = await fetch(sumUrl);
    const data = await safeJson(r);

    return res.json({ title, extract: data?.extract || "" });
  } catch (e) {
    return res.status(500).json({
      error: "wiki-summary failed",
      details: String(e?.message || e),
    });
  }
});

app.get("/api/wd-image", async (req, res) => {
  try {
    const q = String(req.query?.q || "").trim();
    const L = (String(req.query?.lang || "en").trim() || "en").toLowerCase();
